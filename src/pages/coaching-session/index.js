    ...
    <Grid container spacing={6}>
      <Grid item xs={12}>
        <Card>
          ...
          >
            <CardHeader title='Coaching Session' titleTypographyProps={{ variant: 'h6' }} />
            <TextField
              onChange={e => {
          ...
          <TableCoachingSession hrefTo='cs' data={filteredRegistrees} />
        </Card>
      </Grid>
    </Grid>
  )
...
export async function getServerSideProps(ctx) {
  ...
  const res = await fetch(`${process.env.NEXT_PUBLIC_REST_API_URL}/coaching-session`, {
    method: 'GET',
    headers: {
      'x-auth-token': token
    }
